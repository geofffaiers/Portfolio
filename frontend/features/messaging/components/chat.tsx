...
export function Chat({ chatHeader, allowClose, handleCloseChat }: ChatProps) {
  const { user } = useAuthContext()
  const { messages, loading, hasMore, sendMessage, loadMoreMessages, minimized, setMinimized, getSender } = useChat({ chatHeader })
      ...
      <div className={`flex-1 overflow-hidden transition-all duration-300 ${minimized ? "max-h-0" : "max-h-96"}`}>
        <div className="flex-1 p-2 overflow-y-auto h-52">
          ...
          {messages.map((msg, index) => {
            ...
            return (
              <SingleMessage key={`${index}--${msg.id}`} message={msg} isOwnMessage={msg.senderId === user?.id} sender={getSender(msg.senderId)} createdAt={createdAt} showDate={showDate}/>
            )
          })}
        ...
        <div className="flex p-2 border-t">
          ...
          />
          <Button onClick={handleSend} variant="ghost" size="icon" className="ml-2">
...
const SingleMessage = ({ message, isOwnMessage, sender, createdAt, showDate }: SingleMessageProps) => {
