...
import { NewMessage } from '@src/models/sockets/messaging';
...
describe('NewMessage model', () => {
    it('should transform plain object to NewMessage instance with nested Message', () => {
        const plain = newPlainObject(mockMessage());

        const instance = plainToInstance(NewMessage, plain);

        expect(instance).toBeInstanceOf(NewMessage);
